...
unsigned long lastMotionTime = 0;    // 最後一次動作的時間
unsigned long exerciseTime = 0;      // 運動持續時間
const unsigned long TIMEOUT_DURATION = 10000; // 10秒沒動作就超時
bool isExercising = false;          // 是否正在進行運動
...
// Color set
const int redColor[3] = {128, 0, 0};
const int blueColor[3] = {0, 0, 128};
const int purpleColor[3] = {128, 0, 128};
const int yellowColor[3] = {10, 128, 0};
const int greenColor[3] = {0, 128, 0};
...
bool sendExerciseData() {
    ...
    while (!uploadSuccess && retryCount < MAX_RETRIES) {
        ...
        putData["usetime"] = exerciseTime;  // 新增: 回傳運動時間
