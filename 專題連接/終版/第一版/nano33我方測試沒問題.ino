...
// 新增卡片狀態檢查用的 API 參數
const char* cardApiHost = "192.168.137.1";
const int cardApiPort = 3002;
const char* cardApiEndpoint = "/card";
    ...
    if (cardClient.connect(cardApiHost, cardApiPort)) {
        ...
        for (JsonVariant card : cards) {
            if (card["machineid"].as<String>() == machineid) {
                bool condition = card["condition"].as<bool>();
        ...
        cardClient.stop();
        return true;  // 找不到機器 ID 時回傳 true
    }
    
    Serial.println("[卡片檢查] 無法連接到卡片狀態伺服器");
    return true;  // 連線失敗時回傳 true
}
